      ...
      <div className="pt-32">
        ...
        <section className="border-t border-white/5 bg-reelio-black py-32">
          <div className="container mx-auto px-6">
            <div className="grid gap-8 md:grid-cols-12 md:items-start">
              <motion.article initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} className="border border-white/10 bg-white/[0.03] p-8 md:col-span-7 md:p-10">
              ...
              <motion.article initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ delay: 0.1 }} className="border border-white/10 bg-white/[0.02] p-8 md:col-span-5 md:p-10">
            ...
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {aboutMetrics.map((item, idx) => (<motion.div key={item.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.08 }} className="border border-white/10 bg-reelio-dark px-6 py-7">
        ...
        <section className="relative overflow-hidden border-t border-white/5 bg-reelio-dark py-32">
          <div className="pointer-events-none absolute inset-0" style={{
            backgroundImage: "radial-gradient(circle at 12% 10%, rgba(34,211,238,0.18), transparent 38%), radial-gradient(circle at 88% 80%, rgba(251,113,133,0.18), transparent 40%)",
        }}/>
          ...
          <div className="container relative mx-auto px-6">
            ...
            const Icon = philosophyIconMap[item.id] ?? Cog;
            return (<motion.article key={item.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ delay: i * 0.1 }} className="relative overflow-hidden border border-white/10 bg-black/35 p-8 md:p-10" style={{ boxShadow: `inset 0 0 0 1px ${item.accent}4D` }}>
