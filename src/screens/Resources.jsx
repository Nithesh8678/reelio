      ...
      <div className="pt-32">
        ...
        <section className="relative overflow-hidden border-y border-white/5 bg-reelio-black py-24 md:py-32">
          <div className="pointer-events-none absolute inset-0" style={{
            backgroundImage: "radial-gradient(circle at 14% 18%, rgba(227,30,36,0.18), transparent 40%), radial-gradient(circle at 88% 72%, rgba(34,211,238,0.16), transparent 44%), linear-gradient(to bottom, rgba(255,255,255,0.03), transparent 30%)",
        }}/>

          <div className="container relative mx-auto px-6">
            <div className="grid items-center gap-16 lg:grid-cols-12">
              <motion.article initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className="lg:col-span-5">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.65 }} className="relative lg:col-span-7">
                <div className="relative mx-auto aspect-square w-full max-w-[760px]">
                  <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 36, repeat: Infinity, ease: "linear" }} className="absolute inset-[2%] rounded-[2.75rem] border border-white/10"/>

                  <motion.div animate={{ rotate: [360, 0] }} transition={{ duration: 28, repeat: Infinity, ease: "linear" }} className="absolute inset-[8%] rounded-[2.5rem] border border-white/8"/>
                  ...
                  <div className="absolute inset-[14%] overflow-hidden rounded-full border border-white/15 bg-black shadow-[0_0_90px_rgba(227,30,36,0.2)]">
                    <motion.img src="/media-partner.PNG" alt="Production equipment partner" className="h-full w-full object-cover" whileHover={{ scale: 1.04 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}/>
                    <div className="absolute inset-0 bg-gradient-to-tr from-black/35 via-transparent to-white/8"/>
                  </div>

                  <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.5 }} className="absolute right-0 top-[14%] border border-white/15 bg-black/55 px-5 py-4 text-[11px] uppercase tracking-[0.24em] text-white/80 backdrop-blur-sm">
                    Single Image
                  </motion.div>

                  <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.5 }} className="absolute bottom-[14%] left-0 border border-primary/35 bg-primary/15 px-5 py-4 text-[11px] uppercase tracking-[0.24em] text-white">
                    Maximum Impact
                  </motion.div>
                </div>
              </motion.div>
