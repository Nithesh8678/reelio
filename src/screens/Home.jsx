            ...
            const mediaAspectClass = hasVideo
                ...
                : i === 0
                    ...
                    : i === 2
                        ? "aspect-[21/9]"
                        : "aspect-[16/10]";
            return (<motion.div key={work.id} initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ delay: i * 0.14, duration: 0.6 }} whileHover={{ y: hasVideo ? -6 : -10 }} className={`group overflow-hidden border border-white/10 bg-reelio-dark ${spanClass}`}>
                  <div className={hasVideo ? "mx-auto w-full max-w-[340px] px-4 pt-6 md:px-6 md:pt-8" : ""}>
                    <div className={`relative ${mediaAspectClass} overflow-hidden ${hasVideo ? "rounded-[1.75rem] border border-white/15 bg-black p-1" : ""}`}>
                      {hasVideo ? (<iframe src={work.vimeoEmbedUrl} title={work.title} className="h-full w-full rounded-[1.35rem]" loading="lazy" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media; web-share" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/>) : (<>
                          <img src={work.image} alt={work.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" referrerPolicy="no-referrer"/>
