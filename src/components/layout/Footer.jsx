...
export function Footer() {
    ...
    return (<footer className="bg-reelio-black py-20 border-t border-white/5">
      <div className="container mx-auto px-6">
        <div className="mb-20 grid gap-14 md:grid-cols-12">
          <div className="md:col-span-5">
            <Link href="/" className="flex items-center gap-2 mb-8">
              {logoReady ? (<img src="/brand/logo-text.png" alt="Reelio" className="h-10 md:h-11 w-auto object-contain transition-opacity duration-300 hover:opacity-80" onError={() => setLogoReady(false)}/>) : (<span className="brand-wordmark text-2xl text-white">
          ...
          <div className="md:col-span-3">
            ...
            <ul className="space-y-4">
              {footerLinks.map((item) => (<li key={item.name}>
          ...
          <div className="md:col-span-4">
            ...
            <div className="flex gap-6">
              {socialItems.map(({ icon: Icon, href, label }) => (<a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:text-primary hover:border-primary transition-all">
