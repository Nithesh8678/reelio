...
export function Resources() {
    return (<section id="resources" className="py-32 bg-reelio-dark">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {resourcesWorks.map((work, i) => {
            ...
            return (<motion.div key={work.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }} className="group border border-white/10 bg-black/30 p-5">
                ...
                {hasVideo ? (<div className="mx-auto mb-6 w-full max-w-[300px]">
                    <div className="relative aspect-[9/16] overflow-hidden rounded-[1.6rem] border border-white/15 bg-black p-1">
                      <iframe src={work.vimeoEmbedUrl} title={work.title} className="h-full w-full rounded-[1.25rem]" loading="lazy" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media; web-share" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/>
                    </div>
                  </div>) : (<div className="relative mb-6 aspect-[16/10] overflow-hidden">
                    <img src={work.image} alt={work.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer"/>
