...
export function About() {
    return (<section id="about" className="py-32 bg-reelio-dark relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <div className="grid grid-cols-2 gap-12">
              {aboutMetrics.map((stat, i) => (<motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="relative aspect-square">
            <div className="absolute inset-0 border border-primary/20 -translate-x-6 translate-y-6"/>
            <img src="https://picsum.photos/seed/agency/800/800" alt="Reelio Agency" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" referrerPolicy="no-referrer"/>
