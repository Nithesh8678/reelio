...
export function CollaboratorsTicker() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-white/10 bg-reelio-dark/70 py-5">
      ...
      <div className="overflow-hidden">
        <div className="marquee-track flex min-w-max items-center gap-3 px-6">
          {items.map((item, index) => (
            <div
              key={`${item.name}-${index}`}
              className="rounded-full border px-4 py-2 text-xs font-bold tracking-[0.2em] uppercase text-white/90"
              style={{
